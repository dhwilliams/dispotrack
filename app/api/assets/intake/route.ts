...
export async function POST(request: Request) {
  ...
  const cleanedSerial = serialNumber?.trim() || null
  ...
  // 0. Hard-block duplicate serial numbers (Phase 7g — reverses the 5.2b
  //    soft-warning behavior per Amber's feedback). Empty/null serials are
  //    fine; only check when a real value was entered.
  if (cleanedSerial) {
    ...
    if (existing) {
      return NextResponse.json(
        {
          success: false,
          error: `Serial number "${cleanedSerial}" already exists on asset ${existing.internal_asset_id}.`,
          existingAssetId: existing.id,
          existingInternalId: existing.internal_asset_id,
